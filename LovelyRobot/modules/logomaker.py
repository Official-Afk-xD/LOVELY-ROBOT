...
from LovelyRobot import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if not bg_:
        ...
        bg_ = await id_.download_media()
    if not font_:
        fpath_ = glob.glob("./LovelyRobot/resources/fonts/*")
        font_ = random.choice(fpath_)
    if len(name) <= 8:
        fnt_size = 120
        strke = 10
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Lovely Robot](https://t.me/Lovely_Ro_bot)",
    ...
    if os.path.exists(font_):
        if not font_.startswith("./LovelyRobot/resources/fonts"):
            os.remove(font_)
